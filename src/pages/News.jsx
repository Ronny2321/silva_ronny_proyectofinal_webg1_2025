          ...
          <div key={n.id} style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 6 }}>
              {role === "Editor" ? (
                    ...
                    <EditorStatusSelector
                      ...
                      onChange={async (val) => {
                        try {
                          ...
                          setNews((list) =>
                            list.map((it) => (it.id === n.id ? { ...it, estado: val } : it))
                          );
                        } catch (e) {
                ...
                <span style={{ color: "#666", fontSize: 12 }}>Publicado</span>
              )}
            </div>
          </div>
        ))
...
const EditorStatusSelector = ({ current, allowed, saving, disabled, onChange }) => {
  return (
    <label style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
      Estado:
      <select value={current} onChange={(e) => onChange(e.target.value)} disabled={saving || disabled}>
        {allowed.map((s) => (
      ...
      {saving && <span style={{ fontSize: 12, color: "#666" }}>Guardando...</span>}
    </label>
  );
};
